...
public class FlipAnimator {
     ...
     * @param view view on which animation should be performed
     * @param command command to execute on flip
     */
    @TargetApi(11)
	public void doFlip(final View view, final RotationCommand command) {
		...
		if (currentApiVersion >= android.os.Build.VERSION_CODES.HONEYCOMB) {
			...
			rotate1.setAnimationListener(new Animation.AnimationListener() {
				...
				public void onAnimationEnd(Animation animation) {
					...
					view.startAnimation(invisibleToVisible);
				}

				public void onAnimationRepeat(Animation animation) {
				}
			});
			view.startAnimation(visibleToInvisible);
		}
	}
